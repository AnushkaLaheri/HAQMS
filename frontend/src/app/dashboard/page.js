...
export default function Dashboard() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setPatientsPagination({
          ...
          totalPatients: result.data.pagination.totalPatients,
        });
      }
    } catch (e) {
      console.error('[DASHBOARD] Fetch patients error:', e);
    } finally {
  ...
  const fetchDoctorsDropdown = async () => {
    try {
      ...
      console.error('[DASHBOARD] Fetch doctors error:', e);
    }
  };
  ...
  const handleRegisterPatient = async (e) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        setRegMessage(`Error: ${result.error || "Failed to register"}`);
      }
    } catch (err) {
  ...
  const handleBookAppointment = async (e) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        setBookingMessage(`Error: ${result.error || "Failed to book"}`);
      }
    } catch (err) {
  ...
  const handleDeletePatient = async (id) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        alert(`Error: ${result.error || "Unauthorized deletion!"}`);
      }
    } catch (err) {
  ...
  const handleQueueCheckin = async (
    ...
    try {
      ...
      if (res.ok && result.success) {
        setCheckinMessage(
          `Checked in! Generated Token #${result.data.tokenNumber}`,
        );
        if (user?.role === "DOCTOR") fetchDoctorWorklist();
      } else {
        setCheckinMessage(`Error check-in: ${result.error}`);
      }
    } catch (err) {
  ...
  const fetchDoctorWorklist = async () => {
    ...
    try {
      ...
      if (appResult.success) {
        setDoctorAppointments(appResult.data.appointments);
      }
      if (queueResult.success) {
        setDoctorQueue(queueResult.data);
      }
    } catch (e) {
      console.error('[DASHBOARD] Doctor worklist error:', e);
    }
  };
  ...
  const handleUpdateQueueStatus = async (tokenId, newStatus) => {
    try {
      ...
      console.error('[DASHBOARD] Update queue error:', e);
    }
  };
  ...
  const handleCompleteAppointment = async (appId) => {
    try {
      ...
      console.error('[DASHBOARD] Complete appointment error:', e);
    }
  };
  ...
  const generateSystemReport = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (result.success) {
        setAdminReportData(result.data);
      }
    } catch (e) {
  ...
  const searchPhysiciansAdmin = async () => {
    try {
      ...
      console.error('[DASHBOARD] Search physicians error:', e);
    }
  };
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-medium">Loading HAQMS Dashboard...</p>
        </div>
      </div>
    );
  }
